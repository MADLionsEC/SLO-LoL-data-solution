...
from config.constants import API_KEY, SOLOQ, REGIONS, SUPPORTED_LEAGUES
...
def parse_args():
    ...
    return parser.parse_args('-c db -l soloq -r euw -d -e -ta MAD -bt 1-3-2018 -et 2-3-2018'.split(' '))
...
class TestDatabase(unittest.TestCase):
    def setUp(self):
        ...
        try:
            ...
            if args.download:
                print('Downloading.')
                self.current_game_ids, self.new_game_ids = db.get_old_and_new_game_ids(**kwargs)
                db.download_games(current_game_ids=self.current_game_ids, new_game_ids=self.new_game_ids)
                print("\tGames downloaded.")

            if args.export:
                print('Exporting.')
                self.stored_game_ids = db.get_stored_game_ids(**kwargs)
                print('\t{} games found.'.format(len(self.stored_game_ids)))
                if league != SOLOQ:
                    ...
                    ls1 = [str(g[0]) + '_' + str(g[1]) for g in self.stored_game_ids]
                    df = info_df.loc[info_df['gid_realm'].isin(ls1)]
                else:
                    df = pd.DataFrame(self.stored_game_ids).rename(columns={0: 'game_id', 1: 'realm'})
                concatenated_df = db.concat_games(df)
                final_df = concatenated_df
                
                self.row , self.col = final_df.shape
                print(final_df.kills.max())
                print(final_df.kills.min())
    ...
    def test_not_none(self):
        self.assertIsNotNone(self.current_game_ids)
        self.assertIsNotNone(self.new_game_ids)
        self.assertIsNotNone(self.stored_game_ids)
        self.assertIsNotNone(self.final_df)
    def test_df_size(self):
        self.assertEqual(self.row, 40, 'Incorrect ROW size.')
        self.assertEqual(self.col, 222, 'Incorrect COL size.')
    def test_random_values(self):
        self.assertEqual(self.final_df.gameId.max(), 3550276067, 'DataFrame content error (max gameId)')
        self.assertEqual(self.final_df.gameId.min(), 3549167260, 'DataFrame content error (min gameId)')
        self.assertEqual(self.final_df.kills.max(), 17, 'DataFrame content error (max kills)')
        self.assertEqual(self.final_df.kills.min(), 0, 'DataFrame content error (min kills)')
